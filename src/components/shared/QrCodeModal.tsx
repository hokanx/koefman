...
const QrCodeModal = ({ link, open, onClose }: QrCodeModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/50" onClick={onClose}>
      <div className="w-full sm:mx-4 sm:max-w-sm rounded-t-2xl sm:rounded-2xl border border-border bg-card p-6 text-center" style={{ maxHeight: '90dvh', paddingBottom: 'env(safe-area-inset-bottom, 0px)' }} onClick={e => e.stopPropagation()}>
